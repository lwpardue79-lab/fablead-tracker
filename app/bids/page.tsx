...
export default function Bids() {
  ...
  const openBids = bids.filter((bid) => bid.status === "Open" || bid.status === "Submitted");
  ...
  const sourceCount = useMemo(() => new Set(bids.map((bid) => bid.source_url).filter(Boolean)).size || 5, [bids]);
  ...
  return (
    <>
      <PageHeader eyebrow="Opportunity intelligence" title="Live bid opportunities" description="Current, source-linked projects will appear here as regional feeds are connected." action={<button onClick={() => setShowForm((value) => !value)} className="rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">+ Add verified opportunity</button>} />
      ...
      {showForm && (
        <form onSubmit={submitBid} className="card mb-5 grid gap-3 p-5 md:grid-cols-3">
          ...
          <input name="type" className="field" placeholder="Scope / project type" defaultValue="Miscellaneous Metals" />
          ...
          <select name="status" className="field"><option>Open</option><option>Submitted</option><option>Won</option><option>Lost</option><option>No Bid</option><option>Cancelled</option></select>
      ...
      <div className="mb-5 grid gap-3 sm:grid-cols-3"><div className="card p-5"><p className="text-xs font-bold uppercase text-slate-400">Verified open bids</p><p className="mt-2 text-2xl font-semibold">{openBids.length}</p></div><div className="card p-5"><p className="text-xs font-bold uppercase text-slate-400">Weighted value</p><p className="mt-2 text-2xl font-semibold">${Math.round(weightedValue).toLocaleString()}</p><p className="mt-1 text-xs text-slate-400">${totalValue.toLocaleString()} raw value</p></div><div className="card p-5"><p className="text-xs font-bold uppercase text-slate-400">Source feeds planned</p><p className="mt-2 text-2xl font-semibold">{sourceCount}</p></div></div>
