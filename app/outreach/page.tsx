...
export default function Outreach() {
  ...
  return (
    <>
      <PageHeader eyebrow="Activity" title="Outreach" description="Log calls, emails, meetings, portal registrations, notes, other activity, and automatic follow-ups." action={<button onClick={() => setOpen(!open)} className="rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">+ Log outreach</button>} />
      ...
      {open && (
        <form onSubmit={submit} className="card mb-5 grid gap-3 p-5 md:grid-cols-2">
          {fields()}
          <div className="md:col-span-2"><button className="rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">Save activity</button></div>
        </form>
      )}
      <div className="card divide-y divide-slate-100 overflow-hidden">
        {outreachLogs.map((log) => <div key={log.id} className="grid gap-4 p-5 md:grid-cols-[110px_150px_1fr_auto] md:items-start"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{log.date}</p><div className="flex items-start"><Badge tone={log.type === "Call" ? "green" : log.type === "Email" ? "blue" : log.type === "Portal Registration" ? "orange" : "slate"}>{log.type}</Badge></div><div><p className="text-sm font-semibold text-ink">{log.company}</p><p className="mt-1 text-sm leading-relaxed text-slate-600">{log.result || log.notes || "Logged activity"}</p><p className="mt-1 text-xs text-slate-400">{log.contact || "No contact selected"}</p></div><div className="flex flex-col gap-2 md:items-end"><span className="rounded-full bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-500">{log.nextFollowUpDate ? `Follow up ${log.nextFollowUpDate}` : "Logged"}</span><div className="flex justify-end gap-2"><button onClick={() => setEditingId(editingId === log.id ? "" : log.id)} className="rounded-md border px-2 py-1 text-xs font-semibold">Edit</button><button onClick={() => archive(log)} className="rounded-md border border-red-200 px-2 py-1 text-xs font-semibold text-red-700">Delete</button></div></div></div>)}
      ...
      {editingId && outreachLogs.filter((log) => log.id === editingId).map((log) => (
        <form key={log.id} onSubmit={(event) => saveEdit(event, log)} className="card mt-5 grid gap-3 p-5 md:grid-cols-2">
          ...
          {fields(log)}
          <div className="md:col-span-2"><button className="rounded-lg bg-ink px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-slate-800">Save changes</button></div>
        </form>
      ))}
    </>
  );
